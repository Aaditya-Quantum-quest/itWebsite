...
export default function WhyChooseUs() {
  ...
  return (
    <section className="py-24 lg:py-32 bg-bgSecondary overflow-hidden relative">
      {/* Decorative text */}
      ...
      <div className="container mx-auto px-4 md:px-8 lg:px-16 flex flex-col lg:flex-row gap-16 relative z-10">
        ...
        <div className="w-full lg:w-[40%] flex flex-col items-start reveal-left">
          ...
          <h2 className="text-4xl md:text-5xl font-hero font-bold mb-6 !leading-tight">
            Built Different.<br />
            <span className="text-transparent bg-clip-text bg-gradient-cyan-purple">Delivered Better.</span>
          </h2>
          <p className="text-textSecondary mb-8 text-lg font-body leading-relaxed">
        ...
        <div className="w-full lg:w-[60%] grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-12">
          {features.map((item, i) => (
            <div key={i} className="reveal-card relative group p-4 -m-4 rounded-2xl hover:bg-surfaceCard transition-colors">
              <span className="absolute top-0 right-4 text-5xl font-mono font-bold text-surfaceElevated group-hover:text-neonPrimary/10 transition-colors pointer-events-none">
                0{i + 1}
              </span>
              <div className="w-12 h-12 rounded-full bg-neonPrimary/10 flex items-center justify-center mb-4 text-neonPrimary">
                <item.icon className="w-5 h-5" />
              </div>
              <h4 className="text-xl font-heading font-semibold text-textPrimary mb-2 relative z-10">{item.title}</h4>
